...
import { sendDeviceLog, updateDeviceStatus } from "@/src/lib/firebaseLogger";



const WEB_VERSION = "20260520-5";
// 1단계 native video adapter 검증용.
// 운영 tv-play.html에 native 분기를 붙인 뒤에는 기존 tv-login START_URL로 되돌리면 됩니다.
// const START_URL = `https://info-on.cloud/tv-login.html?v=${WEB_VERSION}&nativeHeartbeat=1`;
const START_URL = "https://www.info-on.cloud/tv-play-2.html";
...
type MessageToWeb =
  | {
      type: "NATIVE_VIDEO_STARTED";
      url: string;
      requestId?: string | number;
    }
  | {
      type: "NATIVE_VIDEO_ENDED";
      url: string;
      requestId?: string | number;
    }
  | {
      type: "NATIVE_VIDEO_ERROR";
      url?: string;
      requestId?: string | number;
      message: string;
    }
  | {
      type: "NATIVE_VIDEO_STOPPED";
      requestId?: string | number;
    };
...
function normalizeRequestId(value: unknown): string | number | undefined {
...
export default function HomeScreen() {
  ...
  const currentNativeUrlRef = useRef("");
  const currentRequestIdRef = useRef<string | number | undefined>(undefined);
  ...
  const resetNativeVideoState = useCallback(() => {
    currentNativeUrlRef.current = "";
    currentRequestIdRef.current = undefined;
    setNativeVideoVisible(false);
  }, []);

  const clearNativeVideoSource = useCallback(() => {
    try {
      player.pause();
    } catch (error) {
      console.log("[NATIVE VIDEO] pause failed:", error);
    }

    try {
      player.replace(null);
    } catch (error) {
      console.log("[NATIVE VIDEO] replace null failed:", error);
    }
  }, [player]);

  const postMessageToWeb = useCallback((message: MessageToWeb) => {
  ...
  const stopNativeVideo = useCallback(
    (requestId?: string | number) => {
      playSeqRef.current += 1;

      clearNativeVideoSource();
    ...
    [clearNativeVideoSource, resetNativeVideoState, postMessageToWeb],
  );

  const playNativeVideo = useCallback(
    async (url: string, requestId?: string | number) => {
      const seq = playSeqRef.current + 1;
      playSeqRef.current = seq;

      console.log("[NATIVE VIDEO] play request:", {
        url,
        requestId,
        seq,
      });
      ...
      try {
        ...
        console.log("[NATIVE VIDEO] pause before replace failed:", error);
      }
      ...
      try {
        ...
        if (playSeqRef.current !== seq) {
          console.log("[NATIVE VIDEO] stale replace ignored:", {
            url,
            requestId,
            seq,
            currentSeq: playSeqRef.current,
          });
          return;
        }
        ...
        if (playSeqRef.current !== seq) {
          console.log("[NATIVE VIDEO] stale play ignored:", {
            url,
            requestId,
            seq,
            currentSeq: playSeqRef.current,
        ...
        console.log("[NATIVE VIDEO] started:", {
          url,
          requestId,
          seq,
        });
    ...
    [player, postMessageToWeb, clearNativeVideoSource, resetNativeVideoState],
  );

  useEventListener(player, "playToEnd", () => {
    const endedUrl = currentNativeUrlRef.current;
    const requestId = normalizeRequestId(currentRequestIdRef.current);
  ...
  useEventListener(player, "statusChange", ({ status, error }) => {
    console.log("[NATIVE VIDEO] status:", status, error);

    if (status !== "error") return;

    const failedUrl = currentNativeUrlRef.current;
    const requestId = normalizeRequestId(currentRequestIdRef.current);
    const message =
      error &&
      typeof error === "object" &&
  ...
  const resetToStartUrl = useCallback(
    (reason: string) => {
    ...
    [clearNativeVideoSource, resetNativeVideoState],
  );
  ...
  const reloadWebView = useCallback(
    ...
    [clearNativeVideoSource, resetNativeVideoState, resetToStartUrl],
  );
  ...
  const handleWebViewMessage = useCallback(
    async (event: WebViewMessageEvent) => {
      try {
        ...
        if (data.type === "PLAY_NATIVE_VIDEO" && "url" in data) {
          const requestId =
            "requestId" in data
          ...
          if (typeof data.url === "string") {
            void playNativeVideo(data.url, requestId);
          }

          return;
        }

        if (data.type === "STOP_NATIVE_VIDEO") {
          const requestId =
            "requestId" in data
              ? normalizeRequestId(data.requestId)
              : undefined;

          stopNativeVideo(requestId);
          return;
        }
    ...
    [playNativeVideo, stopNativeVideo, updateCurrentDeviceStatus],
  );
  ...
  return (
    <View style={styles.container}>
      ...
      {isNetworkReady ? (
        <WebView
          ...
          onNavigationStateChange={(navState) => {
            ...
            if (navState.url.includes("tv-play")) {
              isPlayerPageRef.current = true;
            }
      ...
      <View
        pointerEvents={nativeVideoVisible ? "auto" : "none"}
        style={[
          styles.nativeVideoLayer,
          nativeVideoVisible
            ? styles.nativeVideoLayerVisible
            : styles.nativeVideoLayerHidden,
        ]}
      >
        <VideoView
          style={styles.video}
...
const styles = StyleSheet.create({
  ...
  nativeVideoLayerVisible: {
    zIndex: 20,
    opacity: 1,
  },

  nativeVideoLayerHidden: {
    zIndex: 0,
    opacity: 0,
  },
